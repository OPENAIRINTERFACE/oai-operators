...
def create_fn(spec, namespace, logger, patch, **kwargs):
    ...
    conf['nfInterfaces'].update(
                        {'f1cIpaddress': [i['ipv4']['address'].split('/')[0] for i in conf['interfaces'] if i['name']=='f1c'][0],
                         'n2Ipaddress': [i['ipv4']['address'].split('/')[0] for i in conf['interfaces'] if i['name']=='n2'][0],
    ...
    deployment_conf = {'useAdditionalOptions': use_additional_options,'amfIpaddress':conf['amfIpaddress']}
