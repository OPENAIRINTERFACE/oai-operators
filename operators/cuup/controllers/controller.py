...
def create_fn(spec, namespace, logger, patch, **kwargs):
    ...
    svc_status = create_svc(name=f"oai-{NF_TYPE}",#kwargs['body']['metadata']['name'], 
                          ...
                          ports=conf['ports'],
                          kopf=kopf)
    conf['nfInterfaces'].update(
                        {'f1uIpaddress': [i['ipv4']['address'].split('/')[0] for i in conf['interfaces'] if i['name']=='f1u'][0],
                         'n3Ipaddress': [i['ipv4']['address'].split('/')[0] for i in conf['interfaces'] if i['name']=='n3'][0],
                         'e1Ipaddress': [i['ipv4']['address'].split('/')[0] for i in conf['interfaces'] if i['name']=='e1'][0]
        })
    ...
    deployment_conf = {'useAdditionalOptions': use_additional_options,'cucpIpaddress':conf['cucpIpaddress']}
