...
├── Dockerfile  
├── package
│   └── deploy.yaml (Standalone deployment of UDR operator)
├── README.md
...
```bash
docker build -f Dockerfile -t oai-udr-controller:develop. --no-cache
```
...
Make sure you copy operators `yaml` configuration file and network functions `yaml` configuration file from `deployment/udr.yaml` to two different files respectively and configure the env parameters:  
...
```bash
kubectl patch nfdeployments.workload.nephio.org oai-udr -p '{"metadata": {"finalizers": []}}' --type merge
```