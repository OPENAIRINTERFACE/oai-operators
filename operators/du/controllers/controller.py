...
def create_fn(spec, namespace, logger, patch, **kwargs):
    ...
    ## Todo verify the structure of NFConfig
    ...
    svc_status = create_svc(name=f"oai-{NF_TYPE}",#kwargs['body']['metadata']['name'], 
                          ...
                          ports=conf['ports'],
                          kopf=kopf)
    conf['nfInterfaces'].update({'cPlaneMacAdd': [i['mac'] for i in conf['interfaces'] if i['name']=='cplane'][0], 
                       'uPlaneMacAdd': [i['mac'] for i in conf['interfaces'] if i['name']=='uplane'][0]
                    })
    ...
    if conf['RANConfig']['rfSimulated']:
        use_additional_options+=' --rfsim'
    deployment_conf = {'f1duIpaddress': [i['ipv4']['address'].split('/')[0] for i in conf['interfaces'] if i['name']=='f1'][0], 
                       'cucpIpaddress': conf['cucpIpaddress'],
                       'sriovResource': conf['sriovResource'],
                       'useAdditionalOptions': use_additional_options
                    }
