...
NF_TYPE=str(os.getenv('NF_TYPE','du'))      ## Network function name
...
def create_deployment(name: str=None, 
    ...
    :type sa_name: str
    :param deployment_conf: dict
    :type deployment_conf: dict
    :param logger: logger
                    ...
                    "template": {
                      ...
                      "spec": {
                        ...
                        "initContainers": [{
                            ...
                            'sh', 
                            '-c',
                            f"until nc -zv {deployment_conf['cucpIpaddress']} 38472 --sctp ; do echo waiting for cu control plane to start; sleep 1; done" 
                            ]
                        }],
                        "containers": [
                          {
                            ...
                            "env": [
                                ...
                                'value': deployment_conf['useAdditionalOptions']
                                },
                                {'name':'F1_DU_IP_ADDRESS',
                                'value': deployment_conf['f1duIpaddress']
                                },
                                {'name':'CU_CP_IP_ADDRESS',
                                'value': deployment_conf['cucpIpaddress']
                                },
                                {'name':'SRIOV_RESOURCE_NAME',
                                'value': deployment_conf['sriovResource']
                                }
                            ],
                            "resources": {
                                "requests": {
    ...
    if interfaces is not None:
        for interface in interfaces:
            ...
            ## TODO: add a check for sriov resource till then create_nad is a dummy function
...
def create_nad(name: str=None, namespace: str=None, 
    ...
    :param config: configuration 
    :type config: dict
    :param nf_type: nf type 
    :type nf_type: str
    :param sriov: if interface is sriov enabled or not 
    :type sriov: bool
    :param logger: logger
    :type logger: <class 'kopf._core.actions.loggers.ObjectLogger'>
    :param kopf: Instance of kopf
