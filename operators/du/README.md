<table style="border-collapse: collapse; border: none;">
  <tr style="border-collapse: collapse; border: none;">
    ...
    <td style="border-collapse: collapse; border: none; vertical-align: center;">
      <b><font size = "5">OpenAirInterface DU Operator</font></b>
    </td>
  </tr>
</table>
...
├── Dockerfile  
├── package
│   └── deploy.yaml (Standalone deployment of DU operator)
├── README.md
...
DU needs network-attachement-definition and sriov-network to create F1 interface and RU interface respectively. Normally its nephio which will provide the nad. Controller is also capable of creating a nad and sriovnetwork via changing these fields in [nf.yaml](./deployment/nf.yaml) configmap of nf.yaml
...
```bash
    ...
    nad:
      resourceName: 'sriovresource'   #parent interface on the host machine to create the bridge
      create: False    #If false it will wait for multus definition in the cluster namespace
``` 
...
```bash
docker build -f Dockerfile -t oai-du-controller:develop . --no-cache
```
...
```bash
kubectl patch nfdeployments.workload.nephio.org du-edge -n oaicp -p '{"metadata": {"finalizers": []}}' --type merge
```
